...
class TrueBS():
    def __init__(self, args):
        ...
        self.building_mask = rio.open(f"{self.raster_dir}/{self.comune.lower()}_buildings_mask.tif", crs=self.crs)
        self.road_mask = rio.open(f"{self.raster_dir}/{self.comune.lower()}_roads_mask.tif", crs=self.crs)
        with open(f'{self.comune.lower()}.csv') as sacsv:
            self.subareas_csv = list(csv.reader(sacsv, delimiter=','))
        self.big_dsm = rio.open("%s/%s.tif"%(self.raster_dir, self.comune.lower()), crs=self.crs)
        self.big_dsm_raster =  self.big_dsm.read(1)
        self.big_dtm = rio.open("%s/%s_dtm.tif"%(self.raster_dir, self.comune.lower()), crs=self.crs)
        self.big_dtm_raster =  self.big_dtm.read(1)

    def get_building(self, id):
        return self.buildings_df[self.buildings_df.osm_id == id]
    ...
    def get_buildings(self):      
        ...
        if(len(self.buildings)==0):
            ...
            print(f"{len(self.buildings)} buildings, {(self.buffered_area.area*1e-6):.2f} kmq")

    def convert_coordinates(self, coordinates):
        mapped_coord = self.dataset.index(coordinates[0], coordinates[1])
        coordinates_np = np.array([mapped_coord[0], mapped_coord[1], coordinates[2]], dtype=np.uint32)
    ...
    def get_border_points(self, build):
        ...
        building, transform = rio.mask.mask(self.building_mask, [cropped_shape], crop=True)
            ...
            for c in coords:
                ...
                try:
                    if mask[c]:
                        ...
                        z_dsm = self.big_dsm_raster[self.big_dsm.index(*rio.transform.xy(transform, *c))] 
                        # get height of closest point on DTM
                        z_dtm = self.big_dtm_raster[self.big_dtm.index(*rio.transform.xy(transform, *c))]
                        ...
                        if z > 0:
                            ...
                            xx, yy = rio.transform.xy(transform, rows[i], cols[i])
    ...
    def get_area(self, sub_area_id):
        for row in self.subareas_csv:
            if row[1] == sub_area_id: 
                ...
                self.buffered_area = self.sub_area.buffer(self.max_dist/2) #TODO: decide how to manage border effect
                self.road_mask_crop, rm_transform = rio.mask.mask(self.road_mask, [self.buffered_area],crop=True, indexes=1)
                self.translation_matrix, self.inv_translation_matrix = self.gen_translation_matrix(self.road_mask_crop)
                #Crop and save DSM
                raster, transform1 = rio.mask.mask(self.big_dsm, [self.buffered_area], crop=True, indexes=1)
                ...
                #Crop and save DTM
                raster_dtm, transform2 = rio.mask.mask(self.big_dtm, [self.buffered_area], crop=True, indexes=1)
                with MemoryFile() as memfile:
                    new_dataset = memfile.open(driver = 'GTiff',
                ...
                for dens in self.denss:
                    for k in self.ks:
                        for rt in self.ranking_type:
                            if k == 3 or rt == 'r1':
                                self.threestep_heuristic_tnsm(self.buildings, sa_id, ratio, dens, k, rt)
               
    def save_results(self, folder, data):
        os.system(f'mkdir -p {folder}')
        self.save_raster(data.astype(np.int16), f"{folder}/viewshed_uncropped.tif", transform=self.dataset.transform, nodata=-128)
        save_dataset = rio.open(f"{folder}/viewshed_uncropped.tif", 'r', crs=self.crs)
        data, transform1 = rio.mask.mask(save_dataset, [self.sub_area], crop=True, indexes=1)
        self.save_raster(data.astype(np.int16), f"{folder}/viewshed.tif", transform = transform1, nodata=-128)
        array = data.flatten()
        semipositive = array[array>=0]
        np.savetxt(f"{folder}/viewshed.csv", semipositive, fmt='%d')

    def twostep_heuristic_tnsm(self, buildings, sa_id, ratio, dens, k):
        ...
        if ratio < 100:
            ...
            selected_buildings = [b for b in buildings]
        ...
        n_bs = int(self.buffered_area.area * 1e-6 * dens)
        selected_points = set_cover(self.vs.out_global_mem, n_bs, k, ranking_type)
        
        #Recalculate viewshed only on selected points
        self.vs.prepare_cumulative_viewshed(self.dataset_raster)
        viewsheds = np.zeros(shape=(len(selected_points), self.dataset_raster.shape[0], self.dataset_raster.shape[1]), dtype=np.uint8)
        index= []
        os.system(f'mkdir -p {folder}')
        for idx, p_i in enumerate(selected_points):
            ...
            index.append([coord[0], coord[1], coord[2], coord_3003[0], coord_3003[1], build])
            #compute viewshed from this point
            viewsheds[idx] = self.vs.single_viewshed(self.dataset_raster, coord, self.poi_elev, self.tgt_elev, 1)
        ...
        used_buildings = set([all_buildings[(all_coords[p_i][0], all_coords[p_i][1])] for p_i in selected_points])
        
        
        #Save result
        nodata_result = np.where(self.road_mask_crop==0, -128, global_viewshed) 
        self.save_results(folder, nodata_result)
        np.save(f'{folder}/viewsheds', viewsheds)
        #Save metrics
        with open(f'{folder}/metrics.csv', 'w') as fw:
            print(f"{len(self.buildings)} {len(selected_buildings)} {len(used_buildings)} {len(selected_points)}", file=fw)
        #Save index
    ...
    def generate_rankings(self, ks, type=2):
        for k in ks:
            for sa_id in self.sub_area_id:
                for rt in self.ranking_type:
                    self.get_area(sa_id)
    ...
    def single_ranking(self, buildings, sa_id, k):
        ...
        try:
            ...
            for idx, build in enumerate(buildings):
                ...
                coords = [self.convert_coordinates(c) for c in self.get_border_points(build)]
                #save all lists together
                # if coords: #weird buildings may not have any points (see build 675315012)
                coordinates_lists.append(coords) # If i don't append i shift everything by one!
        ...
        selected_buildings = set_cover(self.vs.out_global_mem, len(coordinates_lists), k, ranking_type)
        
        b_ids =  [buildings[i].id() for i in selected_buildings]
    ...
    def twostep_ranking(self, buildings, sa_id, k, ranking_type):
        ...
        try:
            ...
            for idx, build in enumerate(tqdm(buildings)):
                ...
                if len(coords)>self.max_build: #5 by default
                    ...
                    #Initialize matrix for coverred points (for maximal set coverage)
                    #For each k calcualte coverage and update covered points
                    selected_points = set_cover(self.vs.out_global_mem, self.max_build, k, ranking_type, tqdm_enable=False)
                    coordinates_dict[build.osm_id] = [coords[i] for i in selected_points]
                elif len(coords) > 0:
        ...
        n_street_points = self.road_mask_crop.sum()
        selected_buildings = set_cover(viewsheds, len(coordinates_lists), k, ranking_type)
...
if __name__ == '__main__':
    parser = configargparse.ArgumentParser(description='Truenets utility for offline intervisibility', default_config_files=['sim.yaml'])
    ...
    parser.add_argument("--dens", type=int, required=False, action='append', help="BS for sqkm")

    parser.add_argument("--ratio", type=float, required=False, default=[], action='append')

    parser.add_argument("-sa", '--sub_area', required=True, type=str, action='append')
    parser.add_argument("-rt", '--ranking_type', required=True, type=str, action='append')
    parser.add_argument("-k", '--k', required=True, type=int, action='append')
    parser.add_argument("-mb", '--max_build', required=False, type=int, default=5)
