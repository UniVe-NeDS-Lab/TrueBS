...
class TestViewshed(unittest.TestCase):
    ...
    #     viewshed = vs.single_viewshed(dsm, poi_coords, poi_elev, tgt_elev, 0)
    ...
    #     dsm[:,:] = 0
    #     viewshed = vs.single_viewshed(dsm, poi_coords, 1, -2, 0)
    ...
    def test_parallel_translated_viewsheds_single(self):
        ...
        transmat, inv_transmat = vs.gen_translation_matrix(road_mask)
        dsm[:,5] = 2
        vs.parallel_viewsheds_translated(dsm, poi_coords, transmat, road_mask.sum(), poi_elev, tgt_elev, 0)
        t_vs = vs.out_global_mem.copy_to_host()
        viewshed = vs.translate_viewshed(t_vs[:, 0], inv_transmat, dsm.shape)

        res = vs.single_viewshed(dsm, poi_coords[0], poi_elev, tgt_elev, 0)
    ...
    def test_parallel_translated_viewsheds_multiple(self):
        ...
        viewsheds_res = np.zeros(shape=(dsm.shape[0], dsm.shape[1], len(poi_coords)))
        for idx, c in enumerate(poi_coords):
            viewsheds_res[:,:,idx] = vs.single_viewshed(dsm, c, poi_elev, tgt_elev, 0)
        ...
        vs.parallel_viewsheds_translated(dsm, poi_coords, transmat, road_mask.sum(), poi_elev, tgt_elev, 0)
        t_vs = vs.out_global_mem.copy_to_host()
        viewsheds = np.zeros(shape=(dsm.shape[0], dsm.shape[1], len(poi_coords)))
        for idx, c in enumerate(poi_coords):
            viewsheds[:,:,idx] = vs.translate_viewshed(t_vs[:, idx], inv_transmat, dsm.shape)
    ...
    def test_parallel_cumulative_buildings_vs(self):
        ...
        vs.parallel_cumulative_buildings_vs(dsm, transmat, road_mask.sum(), coords_lists, poi_elev, tgt_elev, 0)
        cum_viewshed = vs.out_global_mem.copy_to_host()
        cuda.current_context().memory_manager.reset()

        viewsheds_res = np.zeros_like(cum_viewshed)
        for i, coords in enumerate(coords_lists):
            vs.parallel_viewsheds_translated(dsm, coords, transmat, road_mask.sum(), poi_elev, tgt_elev, 0)
            viewshed = vs.out_global_mem.copy_to_host()
            for j in range(len(coords)):
                viewsheds_res[:,i] = np.bitwise_or(viewsheds_res[:,i], viewshed[:,j])
