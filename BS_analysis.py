...
class BS_Analysis():
    ...
    def load_data(self):
        try:
            self.scalars = np.load(f'{self.base_dir}/{self.strategy}_scalars.npy')
            self.areas = np.load(f'{self.base_dir}/{self.strategy}_areas.npy')
            self.dists = np.load(f'{self.base_dir}/{self.strategy}_dists.npy')
