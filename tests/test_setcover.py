...
class TestSetCover(unittest.TestCase):
    ...
    def test_fast_r1_rank_update(self):
        ...
        fast_r1_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, rank, 2)
        assert np.array_equal(covered_points, np.zeros(shape=10, dtype=np.uint16))
    ...
    def test_fast_r2_rank_update(self):
        ...
        rank = np.zeros(shape=(5), dtype=np.float32)
        viewsheds_old = viewsheds.copy()
        fast_r2_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, rank, 2)
        assert np.array_equal(covered_points, np.zeros(shape=10, dtype=np.uint16))
    ...
    def test_fast_fi_rank_update(self):
        ...
        viewsheds_old = viewsheds.copy()
        fast_fi_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, rank1, rank2, 2)
        assert np.array_equal(covered_points, np.zeros(shape=10, dtype=np.uint16))
    ...
    def test_fast_fi_calc(self):
        ...
        result = [(sum1[i]**2 / (m*sum2[i])) * (sum1[i]/(m*k)) for i in range(len(sum1))]
