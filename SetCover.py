...
def set_cover(viewsheds, n, k, type, tqdm_enable=True):
    covered_points = cuda.to_device(np.zeros(shape=(viewsheds.shape[0]), dtype=np.uint8))
    L = []
    for i in tqdm(range(n)) if tqdm_enable else range(n):
        ...
        set_memory(ranks, 0)
        fi_rank_update[viewsheds.shape[1], 1](viewsheds, covered_points, ranks, k, fix_rank(type))
        ...
        if jstar>=0:
            L.append(jstar)
        update_coverage[viewsheds.shape[0], 1](viewsheds, covered_points, jstar, k)
    return L
...
def set_cover_faster(viewsheds, n, k, type, tqdm_enable=True):
    ...
    for i in tqdm(range(n)) if tqdm_enable else range(n):
        ...
        set_memory(ranks, 0)
        if type == 'r1':
            fast_r1_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, ranks, k)
        elif type == 'r2':
            fast_r2_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, ranks, k)
        elif type == 'fi':
            sum1 = cuda.device_array(shape=viewsheds.shape[1], dtype=np.float32)
            sum2 = cuda.device_array(shape=viewsheds.shape[1], dtype=np.float32)
            set_memory(sum1, 0)
            set_memory(sum2, 0)
            fast_fi_rank_update[blockspergrid, threadsperblock](viewsheds, covered_points, sum1, sum2, k)
            fast_fi_calc[viewsheds.shape[0], 1](sum1, sum2, ranks, k, viewsheds.shape[1])
        for j, ri in enumerate(ranks):
            if ri > r and j not in L:
                r=ri 
                jstar = j
